...
ORDER_FILE = r'C:\Users\Duke Wang\.openclaw\workspace\media\incoming\商城订单2026-05-04_2023Q4.txt'
...
print('历史订单(2023Q4): ' + str(len(rows)) + '条')
...
for r in rows:
    ...
    if not guest:
        ...
        guest = {
            'id': member_id or 'SHOP_2023Q4_' + phone,
            'name': name_clean or '微信用户',
            'phone': phone,
            'source': '小程序商城订单(2023Q4)',
            'notes': '商城客户(2023Q4)',
        }
        guests.append(guest)
        new_guests += 1
...
total_s = '总实付: ' + str(round(total_rev, 2))
date_s = '日期: ' + str(dates[0]) + ' ~ ' + str(dates[-1]) if dates else '日期: N/A'
result_s = '入库: 新客人' + str(new_guests) + ' / 新记录' + str(new_visits) + ' / 去重' + str(dup)
