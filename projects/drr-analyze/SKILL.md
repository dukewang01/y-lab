...
DRR files arrive via Feishu �?`C:\Users\Y\media\inbound\DRR_*.xlsx`
...
```
📊 **{date} 日报**
**🏨 客房**: {sold}�?/ Occ {occ:.1%} / ADR ¥{adr:,.0f}
   RevPAR ¥{revpar:,.0f} | 客房收入 ¥{room_rev:,.0f}（预算¥{bud:,.0f}�?**🍽�?F&B**: ¥{fb_total:,.0f}（预算¥{fb_bud:,.0f}�?
📈 **MTD ({mtd_days}�?**
   出租 {mtd_sold:,}�?/ Occ {mtd_occ:.1%} / ADR ¥{mtd_adr:,.0f}
