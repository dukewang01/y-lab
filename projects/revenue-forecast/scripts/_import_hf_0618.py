"""2026-06-18: Import History & Forecast (6月全月) into FIN station"""
import json, sys
...
fin["meta"]["description"] = "v8.13 + History & Forecast (6月全月30天数据)"

with open(FIN_GRAPH, 'w', encoding='utf-8') as f:
    json.dump(fin, f, ensure_ascii=False, indent=2)

print(f"\nFIN站 History & Forecast 导入完成!")
print(f"  新增HF报告: 1 + 历史日: {history_added} + 预测日: {forecast_added} = {1+history_added+forecast_added}实体")
print(f"  FIN站总计: {len(fin['entities'])}实体 / {len(fin['relations'])}关系 / {fin['meta']['version']}")
print(f"\n📊 6月实际(1-17日): 6,065间 | Occ 65.79% | Rev ¥322.8万 | ADR ¥536.51")
print(f"📊 6月预测(18-30日): 3,018间 | Occ 42.79% | Rev ¥160.9万 | ADR ¥537.70")
print(f"📊 6月全月合计:     9,083间 | Occ 55.82% | Rev ¥483.8万 | ADR ¥536.91")
