...
print(f'\n品项: {len(items)}, 总数�? {total_qty}�?)
print(f'\n�?0�?')
for item in items[:10]:
    print(f'  {item["name"][:20]:20s} | {item["brand"]:10s} | {item["unit"]:4s} | {item["qty"]}�?)
...
hoes = [
    ...
    {"id": "HOE_CONTRACT_JAPANESE_001", "type": "hoe_contract", "label": "HOE00033 日韩式餐具合�?,
     ...
     "total_qty": total_qty, "file": "HOE00033日韩式餐具合同清�?xlsx", "import_date": "2026-05-14"},
]
for h in hoes:
...
print(f'\nFB-HOE总实�? {len(es)}')
print(f'+ 供应�? + 合同1 + {len(items)}品项 = {2+len(items)}')
