...
def deep_search(q, max_per_source=10, max_total=50):
    """全属性深度搜索：name/label/description/type/properties(含嵌套)"""
    # 智能分词
    stop_words = {'的','了','在','是','我','有','和','就','不','人','都','一','一个','上','也','很','到','说','要','去','你','会','着','没有','看','好','自己','这','他','她','它','们','那','什么','怎么','如何','吗','啊','吧','呢','呀','哦','嗯','为','对','与','及','或','但','被','把','从','向','以','于','将'}
...
def classify_question(q):
    ql = q.lower()
    if any(kw in ql for kw in ['评分','扣分','qa','审计','检查','得分','graded']):
        return 'qa_score'
    if any(kw in ql for kw in ['成本率','成本','财务','盈利','收入','花费','利润','drr','收益']):
        if any(kw in ql for kw in ['酒水单','酒单','菜单','茅台','五粮液','红酒','白酒']):
            return 'beverage'
        if '成本' in ql and '酒' in ql:
            return 'cost'
        return 'cost'
    if any(kw in ql for kw in ['空调','设备','维修','工单','mep','工程','给排水','排水','电梯','照明']):
        return 'mep'
    if any(kw in ql for kw in ['投诉','客诉','gsm','纠纷','赔偿','案例','客人不满意']):
        return 'complaint'
    if any(kw in ql for kw in ['酒','白酒','茅台','五粮液','红酒','啤酒','定价','酒单','菜单','菜品']):
        return 'beverage'
    if any(kw in ql for kw in ['客房','房间','入住','2606','房间号']):
        return 'room'
...
def module_qa_score(q):
    ...
    for eid, e in graphs['QA']['entities'].items():
        if 'AREA' in eid and '2025' in eid:
            d = e.get('description','')
            m = re.search(r'Condition扣([0-9.]+)', d)
            if m: areas.append((float(m.group(1)), e.get('name',''), d))
    areas.sort(reverse=True)
    lines.append('\n🔴 扣分最多区域:')
    for pts, name, d in areas[:5]:
        clean = re.search(r'Cleanliness扣([0-9.]+)', d)
        cs = f' + 卫生扣{clean.group(1)}分' if clean else ''
        lines.append(f'  {name}: {pts}分{cs}')
    lines.append('\n🎯 结论: Condition扣分(89.80%)是最大问题')
    return lines
...
def module_cost(q):
    ...
    for oid in targets:
        ...
        if not e: continue
        m3 = prop(e, '3月成本率','无'); m2 = prop(e, '2月成本率','无'); m1 = prop(e, '1月成本率','无')
        budget = prop(e, '预算','无')
        flag = '⚠️' if m3 and budget and isinstance(m3,(int,float)) and isinstance(budget,(int,float)) and m3 > budget else '✅'
        lines.append(f'{flag} {e.get("name","")}: 1月={m1}% → 2月={m2}% → 3月={m3}% (预算={budget}%)')
    return lines
    ...
    for eid, e in graphs['GSM']['entities'].items():
        ...
        for kw in kws:
            if kw in n or kw in d:
                ...
                break
    if found > 8: lines.append(f'  ...及{found-8}条更多')
    lines.append(f'\n📊 GSM共{len(graphs["GSM"]["entities"])}条')
    return lines
...
def module_beverage(q):
    ...
    if '茅台' in ql:
        ...
        if inv:
            ...
            lines.append(f'🥃 飞天53°: ¥{c}/瓶 ×{s}瓶 | 售价¥6,088+15%')
        else: lines.append('⚠️ 数据未录入')
    elif '五粮液' in ql:
        for iid in ['INV_WULIANGYE_52','INV_WULIANGYE_39']:
            e = get_entity(iid,'FIN')
            if e: lines.append(f'🥃 {e.get("name","")}: ¥{prop(e,"cost_price","?")}/瓶')
    else:
        lines.append('📊 定价: 高端白酒40-50% | House Wine ~50% | 整瓶20-25% | 软饮<10%')
        lines.append('💡 可查"茅台"或"五粮液"详情')
    return lines
...
def module_mep(q):
    ...
    if any(kw in ql for kw in ['空调','hvac']):
    ...
    elif any(kw in ql for kw in ['排水','水']):
        ...
        for e in graphs['MEP']['entities'].values():
            d = e.get('description','')
            if any(kw in d for kw in ['未完成','异响','异常']):
    ...
    if '早餐' in ql:
        ...
        for oid in ['OUTLET_OPEN','OUTLET_BQT','OUTLET_YUXI','OUTLET_BACIO','OUTLET_YUAN','OUTLET_ROOM_DINING']:
            ...
            if e:
                ...
                if m3:
                    lines.append(f'  {e.get("name","")}: 3月成本{float(m3)*100:.1f}% | Q1营收¥{float(rev)/10000:.0f}万')
    return lines
...
def module_general(q):
    ...
    if not results:
        ...
        lines.append(f'\n📊 搜索{sum(1 for s in graphs.values() if s["entities"])}站, 命中0条')
        return lines
    ...
    for src, hits in by_source.items():
        lines.append(f'\n📂 {source_names.get(src,src)}站({len(hits)}条):')
        for h in hits[:5]:
    ...
    lines.append(f'\n📊 搜索{sum(1 for s in graphs.values() if s["entities"])}站, 命中{len(results)}条')
    return lines
...
if __name__ == '__main__':
    ...
    print(f'   数据源: {", ".join(k for k,v in graphs.items() if v["entities"])}')
    print('='*60)
    print('   输入问题，/exit退出，/debug看状态')
    print('   示例: "咖啡机奶管残留" "游泳池水质" "早餐温度"')
    print('='*60)
    while True:
        try:
            q = input('\n❓ ').strip()
