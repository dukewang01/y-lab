...
# ===================================================================
lines.append("=" * 80)
lines.append("  📊 效率图谱：谁�?RevPAR 上跑赢了 ADR�?)
lines.append("=" * 80)
lines.append("（RevPAR ÷ ADR = Occ �?越接�?越好，但ADR也重要）")
lines.append("")
...
lines.append("")
lines.append("=" * 80)
lines.append("  🏷�?品牌溢价力：HH vs GI vs DT（同城竞品）")
lines.append("=" * 80)
...
lines.append("")
lines.append("=" * 80)
lines.append("  🏙�?市场结构：酒店密�?vs 价格表现")
lines.append("=" * 80)
lines.append(f"{'市场':<22} {'酒店�?:>6} {'市场ADR':>10} {'旗舰ADR':>10} {'旗舰份额':>10} {'Occ':>7}")
lines.append("-" * 65)
...
lines.append("")
lines.append("=" * 80)
lines.append("  📈 最佳区间：全国HH酒店的ADR分布与营收效�?)
lines.append("=" * 80)
...
lines.append(f"{'ADR区间':<14} {'酒店�?:>6} {'均ADR':>8} {'均Rev':>10} {'均Occ':>8} {'均RevPAR':>10} {'总间�?:>10}")
lines.append("-" * 66)
for l in bracket_labels:
    ...
    if d['count'] > 0:
        ...
        lines.append(f"{l:<14} {d['count']:>6d} {avg_adr:>8.0f} ¥{avg_rev:>8.1f}�?{avg_occ:>7.1f}% {avg_revpar:>9.0f} {d['rns']:>10d}")
...
lines.append("")
lines.append("看数据说话：")
lines.append(f"  �?苏州希尔�?68.5% vs 苏州市场平均 60.1% �?高于市场8.4pp，不�?)
lines.append(f"  �?苏州希尔�?68.5% vs 全国HH平均 ~75% �?但低于同�?.5pp")
lines.append(f"  �?苏州希尔�?68.5% vs Yinshan Lake 68.9% �?本地竞品居然还高0.4pp")
lines.append("")
lines.append("可能的原因：")
...
lines.append("  它们Occ普遍�?5-85%，我�?8.5%明显偏低�?)
lines.append("  Occ提升6.5pp�?5% = 多销5,278间夜 = ¥343万增收（YTD�?)
...
for line in content.split('\n'):
    ...
    try:
        print(line.replace('📊','').replace('🏷�?,'').replace('🏙�?,'').replace('📈','').replace('🎯','').replace('🔎','').replace('🔥',''))
    except:
        pass
