#!/usr/bin/env python3
"""Deep dive: Compare City-X market vs peers, and our Hotel-A vs peer HH hotels."""
...
# 1. Peer cities â€?similar scale to City-X
print("=" * 80)
print("  ã€è‹å·?vs å¯æ¯”åŸŽå¸‚ã€?)
print("=" * 80)
peers = ['Jiangsu/City-X', 'Jiangsu/Nanjing', 'Zhejiang/Hangzhou', 
         'Fujian/Xiamen', 'Sichuan/Chengdu', 'Chongqing', 'Tianjin',
         'Hubei/Wuhan', 'Shaanxi/Xi\'an', 'Hunan/Changsha']
print(f"{'åŸŽå¸‚':<22} {'ADR':>8} {'Rev(ä¸?':>10} {'RevPAR':>8} {'RNs':>8} {'Occ':>7} {'ADRæº¢ä»·':>8}")
print("-" * 72)
sz_adr = markets['Jiangsu/City-X']['adr']
for city in peers:
    if city in markets:
        m = markets[city]
        premium = (m['adr'] - sz_adr) / sz_adr * 100
        flag = " <<< è‹å·ž" if 'City-X' in city else ""
...
print()
print("=" * 80)
print("  ã€è‹å·žå¸Œå°”é¡¿ vs å…¨å›½å¯æ¯”HHé…’åº—ã€?)
print("=" * 80)
...
print(f"{'æŽ’å':>3} {'é…’åº—':<40} {'æ‰€åœ¨å¸‚åœ?:<18} {'ADR':>8} {'Rev(ä¸?':>10} {'RevPAR':>8} {'RNs':>7} {'Occ':>7}")
print("-" * 100)
...
lines = []
lines.append("=" * 80)
lines.append("  ã€è‹å·?vs å¯æ¯”åŸŽå¸‚ã€?)
lines.append("=" * 80)
lines.append(f"{'åŸŽå¸‚':<22} {'ADR':>8} {'Rev(ä¸?':>10} {'RevPAR':>8} {'RNs':>8} {'Occ':>7} {'ADRæº¢ä»·':>8}")
lines.append("-" * 72)
for city in peers:
    if city in markets:
        m = markets[city]
...
lines.append("")
lines.append("=" * 80)
lines.append("  ã€è‹å·žå¸Œå°”é¡¿ vs å…¨å›½å¯æ¯”HHé…’åº— Top20ã€?)
lines.append("=" * 80)
lines.append(f"{'æŽ’å':>3} {'é…’åº—':<40} {'æ‰€åœ¨å¸‚åœ?:<18} {'ADR':>8} {'Rev(ä¸?':>10} {'RevPAR':>8} {'RNs':>7} {'Occ':>7}")
lines.append("-" * 100)
for i, h in enumerate(hh_hotels[:20], 1):
    flag = " <<<" if h == our else ""
    mkt_short = h['market'].split('/')[-1] if '/' in h['market'] else h['market']
